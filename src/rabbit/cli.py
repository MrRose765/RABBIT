...
def cli(
    ...
    if input_file is None and len(contributors) == 0:
        typer.echo("Error: Provide either contributor names or an input file.", err=True)
        raise typer.Exit(code=1)

    if key is None or len(key) < 40:
        typer.echo("Warning: A valid GitHub API key is recommended for higher rate limits.", err=True)
